...
class SparkParquetReadAndWriteJavaExample implements Serializable {
    ...
    public static DataFrame getDataFrame(SQLContext sqlContext) {
        List<Employee> employeeList = getEmployeeData();
        DataFrame employeeDF = sqlContext.createDataFrame(employeeList, Employee.class);
        display(employeeDF);
        return employeeDF;
    }
    ...
    public static void display(DataFrame dataFrame) {
        // printSchema
