 ...
 */
object MyInProcessLauncher extends App with Serializable {

  @transient lazy val logger: Logger = Logger.getLogger(getClass.getName)

  if (args.length < 4) {
    logger.error("MyInProcessLauncher <master> <deploy_mode> <class_name> <resource_location> <appArgs>")
    logger.info("MyInProcessLauncher <yarn> <cluster> <org.apache.spark.examples.SparkPi> </path/to/spark-examples_2.11-2.3.2.3.1.5.0-152.jar> ")
    System.exit(0)
  }
